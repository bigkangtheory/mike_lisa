    ...
    for ( ; ( node = nodes[ i ] ) != null; i++ ) {
      ...
      if ( node.parentNode ) {
        ...
        $('.cart-item').each(function(index){
          $($('.cart-item')[index]).data('cartitem-line', index + 1);
        })
...
Shopify.changeItemByLine = function(t, r, e) {
    var a = {
        type: "POST",
        url: "/cart/change.js",
...
var RichText = function () {
  function RichText($el) {
    ...
    if (this.$el.length) {
      this.groupedContent = new index$2($el.get(0), {
        layout: 'tabs',
        intelliparse: false
      });
      index$1();
...
var CurrencyConverter = function () {
  ...
  createClass$2(CurrencyConverter, [{
    ...
    value: function update(priceEl) {
      ...
      var moneyValue = this._moneyValue(priceEl.innerHTML);
      var centsValue = this._centsValue(moneyValue, this.options.moneyFormat, this.options.shopCurrency);
      var formattedPrice = this._formatMoney(centsValue, this.options.moneyFormat);
    ...
    key: '_formatWithDelimiters',
    value: function _formatWithDelimiters(number) {
    ...
    key: '_centsValue',
    value: function _centsValue(moneyAmount, format, currency) {
      var cents = 0;
    ...
    value: function _convertEl(priceEl, oldCurrency, newCurrency) {
      ...
      if (priceEl.getAttribute('data-currency-' + newCurrency)) {
        ...
        var moneyValue = this._moneyValue(priceEl.innerHTML);
        var cents = window.Currency.convert(this._centsValue(moneyValue, oldFormat, oldCurrency), oldCurrency, newCurrency);
        var newFormattedAmount = this._formatMoney(cents, newFormat);
...
var index$3 = CurrencyConverter$1;
...
var HeaderSearch = function () {
  function HeaderSearch($els, settings) {
    ...
    this.events = [this.$button.on('click.header-search', function (event) {
      event.preventDefault();
      _this._search(event);
    }), this.$input.on('keyup.header-search', index$4(this._search, 250)), this.$input.on('focus.header-search', this._searchFocused), this.$takeOverButton.on('click.header-search', this._toggleTakeOver)];
  }
  ...
  createClass(StaticSectionHeader, [{
    ...
    value: function _converter() {
      ...
      this.$converter.on('change.converter', function (event) {
        ...
        index$3.setCurrency(event.currentTarget.value);
      });
    }
  }, {
...
var MessageBanner = function () {
  ...
  createClass(MessageBanner, [{
    ...
    value: function _bindEvents($banner) {
      ...
      }), jquery$1(window).on('resize.message-banner', index$4(function () {
...
var StaticCart = function () {
  ...
  createClass(StaticCart, [{
    ...
    value: function _bindEvents() {
      ...
      this.$el.on('quantity:error', '[data-quantity-wrapper]', index$4(function (event, data) {
        _this2._quantityError(data.limit);
      }, 30, false, true));

      this.$window.on('resize.cart-page', index$4(function () {
    ...
    value: function _updateCart($cartItem, quantity) {
      var _this3 = this;
      var line = $cartItem.data('cartitem-line');
      var key = $cartItem.data('cartitem-key');

      var id = $cartItem.data('cartitem-id');

      // Notify Shopify updated item
      Shopify.changeItemByLine(line, quantity, function (response) {
...
var VanillaModal = unwrapExports(index$5);
...
var Modal = function () {
  ...
  createClass(Modal, [{
    ...
    value: function _onOpen() {
      ...
      this.$window.on('resize.modal', index$4(function () {
  ...
  createClass(AddToCartFlyout, [{
    ...
    value: function _updateCart($atcButton) {
      ...
      jquery$1.ajax({
        ...
        $flyOut.find('[data-atc-banner-product-title]').html(response.product_title + response.properties_formatted + ' \xD7 ' + quantity);
        ...
        jquery$1.ajax({
          ...
          $subTotal.text(Shopify.formatMoney(response.total_price, _this.settings.moneyFormat));
          index$3.update($subTotal[0]);
    ...
    value: function _bindEvents($flyOut) {
      ...
      this.events.push([$flyOut.on('click.atc-banner', '[data-atc-banner-close]', function (event) {
        ...
        index$4(_this2._positionFlyOut($flyOut), 250);
...
var ProductDetails = function () {
  ...
  createClass(ProductDetails, [{
    ...
    value: function _updatePrice(variant) {
      ...
      index$3.update(this.variantFields.$compareAtPriceMoney[0]);

      // Update price
      this.variantFields.$priceMoney.text(Shopify.formatMoney(variant.price, this.settings.money_format));
      index$3.update(this.variantFields.$priceMoney[0]);
    }
  }, {
    key: '_updateBadge',
    value: function _updateBadge(variant) {
      ...
      if (this.variantFields.$badgeMoneySaved.length) {
        ...
        index$3.update(this.variantFields.$badgeMoneySaved[0]);
      }
    ...
    value: function _unloadGallery() {
      if (this.gallery) {
        ...
        this.gallery = null;
        this.$slider.find('[data-product-image]').attr('style', '');
      }
    }
  }, {
    key: '_initGallery',
    value: function _initGallery() {
      ...
      imagesloaded(this.$slider[0], function () {
        _this2.gallery = new index(_this2.$slider[0], flickityOptions);
    ...
    value: function _bindGallery() {
      ...
      this.$window.on('resize.gallery', index$4(function () {
        _this3.$slider.trigger('heightUpdate.gallery');
      }));
    }
  }, {
...
var DynamicTwitterFeed = function () {
  function DynamicTwitterFeed(section) {
    ...
    this.$window.on('resize.twitter', index$4(function () {
...
function instagramSquared(photo) {
  ...
  // Crop each size
  response.large = getSquareUrl(response.large, photo, 'standard_resolution');
  response.small = getSquareUrl(response.small, photo, 'low_resolution');
 ...
 */
function getSquareUrl(url, photo, image) {
  var templateUrl = photo.images.thumbnail.url;
  var sizes = url.match(/\/[ps]([0-9]+)x([0-9]+)\//);

  // The target image URL does not have size information...
  if (!sizes || sizes.length < 3) {
    // In the worst case, return the thumbnail URL as a fallback
    if (!photo.images[image]) return templateUrl;

    // But try to pull the target resolution size from the photo object otherwise
    var templateEdge = Math.min(
      photo.images.thumbnail.width,
      photo.images.thumbnail.height
    );

    var edge = Math.min(
      photo.images[image].width,
      photo.images[image].height
    );

    return templateUrl.replace(templateEdge + 'x' + templateEdge, edge + 'x' + edge);
  }

  // Best case: use the target image URL sizes.
...
var DynamicInstagramFeed = function () {
  function DynamicInstagramFeed(section) {
    ...
    this.squareImages = this.settings.square_images;
  ...
  createClass(DynamicInstagramFeed, [{
    ...
    value: function _renderPhotos(photos) {
      ...
      photos.forEach(function (photo) {
        ...
        var squareSizes = index$6(photo);

        var srcSmall = _this2.squareImages ? squareSizes.small : lowResolution.url;
        var srcLarge = _this2.squareImages ? squareSizes.large : standardResolution.url;

        var $link = jquery$1('<a target="_blank" href="' + photo.link + '"></a>');
        var $photo = jquery$1('<img src="' + srcSmall + '">');
